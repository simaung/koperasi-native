<?php
	...
	//$kode_anggota = ($kode_anggota = isset( $_POST['kode_anggota']))? $kode_anggota: isset($_GET['kode_anggota']);
	//$nama_anggota = ($nama_anggota = isset( $_POST['nama_anggota']))? $nama_anggota: isset($_GET['nama_anggota']);
	//$alamat_anggota = ($alamat_anggota = isset( $_POST['alamat_anggota']))? $alamat_anggota: isset($_GET['alamat_anggota']);
	//$jenis_kelamin = ($jenis_kelamin = isset( $_POST['jenis_kelamin']))? $jenis_kelamin: isset($_GET['jenis_kelamin']);
	//$pekerjaan = ($pekerjaan = isset( $_POST['pekerjaan']))? $pekerjaan: isset($_GET['pekerjaan']);
	//$tgl_masuk = ($tgl_masuk = isset( $_POST['tgl_masuk']))? $tgl_masuk: isset($_GET['tgl_masuk']);
	//$telp = ($telp = isset( $_POST['telp']))? $telp: isset($_GET['telp']);
	//$tempat_lahir = ($tempat_lahir = isset( $_POST['tempat_lahir']))? $tempat_lahir: isset($_GET['tempat_lahir']);
	//$tgl_lahir = ($tgl_lahir = isset( $_POST['tgl_lahir']))? $tgl_lahir: isset($_GET['tgl_lahir']);
	//$u_entry = ($u_entry = isset( $_POST['u_entry']))? $u_entry: isset($_GET['u_entry']);
	//$tgl_entri = ($tgl_entri = isset( $_POST['tgl_entri']))? $tgl_entri: isset($_GET['tgl_entri']);
	...
	switch ($pros){
		...
		$tunggak=mysqli_num_rows(mysqli_query($koneksi,"SELECT * from t_pinjam where kode_anggota='$kode' and status='belum lunas' order by kode_pinjam desc "));
		if($tunggak>0)
		{ ?>
			<script>alert("Maaf Aggota ini masih ada tunggakan");window.location="../index.php?pilih=1.2";</script>
		<?php }
