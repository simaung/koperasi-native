...
<!-- PINJAMAN -->
	<script language="JavaScript">
		// fungsi untuk get besar_pinjaman
...
<?php
		...
		$kode_jenis = isset($_GET['kode_jenis_simpan']) ? $_GET['kode_jenis_simpan'] : '';
...
<div class="row mt">
    ...
    <form action="transaksi/proses_transaksi.php?pros=simpan" method="post" id="form" name="mainform" onSubmit="validasiSimpan()">
        ...
        <div class="form-group">
            ...
            <input type="text" name="user_entri" class="form-control" size="54" value="<?php if(!isset($_SESSION)){session_start();} echo $_SESSION['kopname'];?>" readonly >
        </div>
        <div class="form-group">
            <label>Tanggal Entri</label>
            <input type="text" name="tgl_entri" class="form-control" size="54" value="<?php echo date("Y-m-d");?>" />
        </div>
...
<div class="form-group">
    <label>User Entri</label>
    <input type="text" class="form-control" name="u_entry" size="54" value="<?php if(!isset($_SESSION)){session_start();} echo $_SESSION['kopname'];?>" readonly>
</div>
<div class="form-group">
    <label>Tanggal Entri</label>
...
<?php
	}else if($aksi=='angsur')
	{
		...
		$kodep = isset($_GET['kode_pinjam']) ? $_GET['kode_pinjam'] : '';
...
{ $kode = isset($_GET['kode_anggota']) ? $_GET['kode_anggota'] : ''; $anggota=mysqli_fetch_array(mysqli_query($koneksi,"SELECT nama_anggota from t_anggota where kode_anggota='$kode'")); ?>
	<div class="row mt">
  <div class="col-lg-12">
     <div class="form-panel">
        ...
        <?php $df=mysqli_fetch_array(mysqli_query($koneksi,"SELECT * FROM t_pinjam where kode_anggota='$kode' order by kode_pinjam desc"));
